...
public class Exam1_43 {
  ...
  public static void main(String[] args) {
    ...
    float r3 = f1 + f2; 
    //f1과 f2에 들어있는 값이 유효자릿수라 하더라도 연산 결과가 유효자릿수가 아니라면 값이 깨진다.
    //float과 float의 연산결과는 float이기 때문
    
    float r1 = f1 + f2;
    
    System.out.println(r3);
    double r2 = f1 + f2;
    System.out.println(r2); // 988.7654

    // 기대값 : 987.6543 + 1.111111 = 988.765411
    // 결과값 : 988.7654
    // 기대한 결과가 나오지 않은 이유
    // float과 float의 연산 결과는 float이다.
    // > double 변수에 저장하기 전에 이미 float값이 되면서 일부값이 왜곡
    ...
    // 4byte float 부동소수점을 8byte double 부동소수점 변수에 저장할 때 뒤의 왜곡된 값이 들어 갈 수 있다.
    // 가능한 한 double 변수로 값을 바꾼 다음에 연산을 수행하라.
    // double변수를 처음부터 사용하는것을 권장

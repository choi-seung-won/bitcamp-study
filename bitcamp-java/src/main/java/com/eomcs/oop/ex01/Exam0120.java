...
public class Exam0120 {
  public static void main(String[] args) {
    ...
    // => 이 선언은 설계도에 따라 메모리를 준비시키는 명령이 아님.
    // => score에따라 만든 메모리의 주소를 저장할 변수를 만드는 명령이다.
    // => 이렇게 메모리의 주소를 저장하는 변수를 "reference"라 부름
    //int와같은 primitive data type인 경우
    //위와같이 변수 선언하나 class설계 메모리는 이런 방식으로 만들 수 없음.
    
    //class 문법으로 설계한 메모리 준비
    // => new classname();
    // => score 설계도에 따라 메모리를 생성(확보)한다.
    // => 이렇게 설계도에 따라 준비된 메모리를 "instance" 라 부름
    // => 확보된 메모리를 사용하려면 주소를 보관할 필요가 있다.
    // => 위에서 준비한 레퍼런스에 주소를 보관.
    
    s = new Score(); 
    // Score 설계도에 따라 메모리를 확보하고,
    // 그 메모리의 주소를 s에 저장한다.   

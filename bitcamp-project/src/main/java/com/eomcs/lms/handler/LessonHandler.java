...
public class LessonHandler {
  //instance field, 각 수업목록을 개별적으로 관리
  ...
  public static void listLesson(LessonHandler lessonHandler) { // classvar referencevar
    for (int i = 0; i < lessonHandler.Member_Count; i++) {
      Lesson lec = lessonHandler.lecture[i]; // 변수 중복을 막기위하여 var네임 변경
  ...
  //  lecture[Member_Count++] = lec; // lec을 count++로 증가한 번호의 배열에 저장
    ...
    lessonHandler.lecture[lessonHandler.Member_Count++] = lec;
  ...
  public static void detailLesson(LessonHandler lessonHandler) {
    ...
    Lesson lec = null;
    for(int i = 0; i < lessonHandler.Member_Count; i ++) {
      if (lessonHandler.lecture[i].no == no) {
        lec = lessonHandler.lecture[i];
        break;
      }
    }
